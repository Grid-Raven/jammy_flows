...
def _draw_legacy_standard_normal(num_samples, dim, dtype, device, seed):
    # Preserve the established eager-mode values and NumPy RNG semantics.
    if seed is not None:
        numpy.random.seed(seed)

    values = numpy.random.normal(size=(num_samples, dim))
    return torch.from_numpy(values).to(dtype=dtype, device=device)


def draw_standard_normal(num_samples, dim, dtype, device, seed=None):
    if not _is_compiling():
        return _draw_legacy_standard_normal(
            num_samples,
            dim,
            dtype,
            device,
    ...
    if _seeded_standard_normal is None:
        raise RuntimeError(
            "Seeded sampling under torch.compile requires torch.library.custom_op support."
        )
